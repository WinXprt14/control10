...
cls
set subr=1 > nul
title %Username%: Control Prompt Homepage (Subroutine: %SUBR%) && color 3 && color %clrdef%
...
if not "%version%" == "10.0" (echo NOTE: THE VERSION OF WINDOWS YOU ARE USING IS OLDER OR NEWER THAN WINDOWS 10.)
echo.
...
echo.
echo Customize desktop icons (PRESS I)
echo.
echo Settings that you probably didn't know existed (PRESS J)
echo.
set /p F=Your Option: 
...
if %F% == h (start rundll32 shell32.dll Control_RunDLL desk.cpl,,1 && goto F)
if %F% == I (start rundll32 shell32.dll Control_RunDLL desk.cpl,,5 && goto F)
if %F% == i (start rundll32 shell32.dll Control_RunDLL desk.cpl,,5 && goto F)
if %F% == J (goto FJ)
if %F% == j (goto FJ)
if %F% == Z (goto 1)
...
echo.
set /p FJ=Your Option: 
if %FJ% == A (reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v LastActiveClick /t REG_DWORD /d 1 /f > nul && goto FJ)
if %FJ% == a (reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v LastActiveClick /t REG_DWORD /d 1 /f > nul && goto FJ)
if %FJ% == B (reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v LastActiveClick /t REG_DWORD /d 0 /f > nul && goto FJ)
if %FJ% == b (reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v LastActiveClick /t REG_DWORD /d 0 /f > nul && goto FJ)
if %FJ% == C (
cls && for /f "tokens=3" %%a in ('reg query "HKCU\Control Panel\Desktop"  /v Wallpaper  ^|findstr /ri "REG_SZ"') do set wlploc=%%a
echo WallPaper image location: %wlploc% && start %wlploc%\..\ && pause && goto FJ
)
if %FJ% == c (
cls && for /f "tokens=3" %%a in ('reg query "HKCU\Control Panel\Desktop"  /v Wallpaper  ^|findstr /ri "REG_SZ"') do set wlploc=%%a
echo WallPaper image location: %wlploc% && start %wlploc%\..\ && pause && goto FJ
...
if %FJ% == 1 (goto 1)
if %FJ% == 0 (goto 0)
if %FJ% == LCK (rundll32 user32.dll LockWorkStation && goto FJ)
if %FJ% == lck (rundll32 user32.dll LockWorkStation && goto FJ)
if %FJ% == WINAERO (start https://winaero.com/ && goto FJ)
if %FJ% == winaero (start https://winaero.com/ && goto FJ)
if %FJ% == Winaero (start https://winaero.com/ && goto FJ)
if %FJ% == WinAero (start https://winaero.com/ && goto FJ)
if %FJ% == winAero (start https://winaero.com/ && goto FJ)
if %FJ% == RESEXP (taskkill /f /im explorer.exe > nul && ping localhost -n 2 > nul && start explorer.exe && goto FJ)
if %FJ% == resexp (taskkill /f /im explorer.exe > nul && ping localhost -n 2 > nul && start explorer.exe && goto FJ)
if %FJ% == D (goto FJD)
if %FJ% == d (goto FJD)
if %FJ% == E (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v TaskbarAnimations /t REG_DWORD /d 1 /f > nul && goto FJ)
if %FJ% == e (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v TaskbarAnimations /t REG_DWORD /d 1 /f > nul && goto FJ)
if %FJ% == F (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v TaskbarAnimations /t REG_DWORD /d 0 /f > nul && goto FJ)
if %FJ% == f (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v TaskbarAnimations /t REG_DWORD /d 0 /f > nul && goto FJ)
if %FJ% == G (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v DisablePreviewWindow /t REG_DWORD /d 0 /f > nul && goto FJ)
if %FJ% == g (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v DisablePreviewWindow /t REG_DWORD /d 0 /f > nul && goto FJ)
if %FJ% == H (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v DisablePreviewWindow /t REG_DWORD /d 1 /f > nul && goto FJ)
if %FJ% == h (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced /v DisablePreviewWindow /t REG_DWORD /d 1 /f > nul && goto FJ)
cls && color 2 && color %clrdef%
echo Bad option or a problem caused to batch!
pause && goto FJ
:FJD
cls && echo [PRESS C TO CANCEL / TO RESTORE DEFAULT TEXT PRESS D / ENTER TO CONFIRM]
...
if "%DC%" == "Z" (goto FJ)
if "%DC%" == "z" (goto FJ)
if "%DC%" == "D" (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search\Flighting /v RotateFlight /t REG_DWORD /d 0 /f > nul && reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search\Flighting\0\SearchBoxText /v Value /t REG_SZ /d "" /f > nul && goto FJ)
if "%DC%" == "d" (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search\Flighting /v RotateFlight /t REG_DWORD /d 0 /f > nul && reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search\Flighting\0\SearchBoxText /v Value /t REG_SZ /d "" /f > nul && goto FJ)
cls && echo "%DC%" Will be set as search box text if you continue. Continue? [YES (Y) / NO (ANY OTHER LETTER)] && echo.
set /p DC1=Your option: 
if "%DC1%" == "Y" (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search\Flighting /v RotateFlight /t REG_DWORD /d 1 /f > nul && reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search\Flighting\0\SearchBoxText /v Value /t REG_SZ /d "%DC%" /f > nul && goto FJ)
if "%DC1%" == "y" (reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search\Flighting /v RotateFlight /t REG_DWORD /d 1 /f > nul && reg add HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Search\Flighting\0\SearchBoxText /v Value /t REG_SZ /d "%DC%" /f > nul && goto FJ)
goto FJ
:FG
cls
...
if %I% == a (start shell:::{c58c4893-3be0-4b45-abb5-a63e4b8c8651} && goto I)
if %I% == B (
md %USERPROFILE%\Desktop\YourFolderIsReady.{ED7BA470-8E54-465E-825C-99712043E01C} > nul
cls && color f && color %clrdef% && echo Look for a folder with control panel icon without name in your desktop && pause && start shell:::{3080F90D-D7AD-11D9-BD98-0000947B0257} > nul && goto I
)
if %I% == b (
md %USERPROFILE%\Desktop\YourFolderIsReady.{ED7BA470-8E54-465E-825C-99712043E01C} > nul
cls && color f && color %clrdef% && echo Look for a folder with control panel icon without name in your desktop && pause && start shell:::{3080F90D-D7AD-11D9-BD98-0000947B0257} > nul && goto I
...
if %I% == 1 (goto 1)
if %I% == 0 (goto 0)
cls && color 1 && color %clrdef% && echo Bad option or a problem caused to batch! && pause && goto I
cls
color 2 && color %clrdef%
echo Bad option or a problem caused to batch!
pause && goto I
...
echo background color (WITHOUT ANY SPACES), and then type the number/letter of the color you'd like to set
...
echo            03 is the default color of the second version of Control Prompt
echo                 When none color is set the text will be blue by default
echo   (If the program was run by command prompt drag-n-drop the default color will be white text on 
echo                              black background)
echo                          42 produces green text on red background
echo.
echo.
if defined %clrdef% (echo The color is set to %clrdef%) else (echo.)
echo NOTE: Numbers 00,11,22,33,44,55,66,77,88,99 CANNOT be set && echo. && echo.
echo To restore default colors (PRESS A)
echo.
echo To restore previous color configuration (PRESS B)
echo.
if defined %clrdef% (echo Note that the default color has been changed && echo.)
...
if %ID% == A (set clrdefbackup=%clrdef% && set "clrdef=" && goto ID)
if %ID% == a (set clrdefbackup=%clrdef% && set "clrdef=" && goto ID)
if %ID% == B (set clrdef=%clrdefbackup% && set "clrdefbackup=" && goto ID)
if %ID% == b (set clrdef=%clrdefbackup% && set "clrdefbackup=" && goto ID)
if %ID% == Z (goto I)
if %ID% == z (goto I)
if %ID% == 1 (goto 1)
